...
class MessageDBService {
	...
	public getMessage = async ({
		...
		const result = (
			await client.execute(this.GET_MESSAGE_QUERY, [messageId], {
				prepare: true,
			})
		).first();
	 ...
	 */
	public decreaseUnreadCount = async ({
		chatId,
		userId,
	}: {
		chatId: types.Uuid;
	 ...
	 */
	public increaseUnreadCount = async ({
		chatId,
		userIds,
	}: {
		chatId: types.Uuid;
	 ...
	 */
	public deleteChatUnreadCounters = async ({
		chatId,
		userIds,
	}: {
		chatId: types.Uuid;
		...
		try {
			await Promise.all(
				userIds.map((userId) =>
					client.execute(
						`DELETE FROM app_keyspace.user_unread_count WHERE chat_id = ? AND user_id = ?`,
						[chatId, userId],
						{ prepare: true }
					)
				)
	...
	public getUnreadMessagesCount = async ({
		userId,
		chatId,
	}: {
		userId: types.Uuid;
		...
		const result = (
			await client.execute(
				this.GET_UNREAD_MESSAGES_COUNT_QUERY,
				[userId, chatId],
				{ prepare: true }
			)
		).first();
	...
	/**
	 * Get the last message sent by a user in a chat
	 */
	public getLastMessageByUserAndChat = async ({
		chatId,
		userId,
	}: {
		chatId: types.Uuid;
		...
		const query = `SELECT * FROM messages WHERE chat_id = ? AND user_id = ? LIMIT 1;`;
		const result = (
